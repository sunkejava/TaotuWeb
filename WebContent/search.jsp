...
<c:set var="Sitesearch" value="%<%=search %>%"/>
     <sql:query dataSource="${snapshot}" var="result2">
SELECT CASE WHEN MOD (COUNT(*),15) > 0 THEN COUNT(*) DIV 15 +1 ELSE COUNT(*) DIV 15 END AS Pages 
FROM db_taotu WHERE tagName LIKE '<%=searcha %>' or name LIKE '<%=searcha %>';
</sql:query>
<c:forEach var="row2" items="${result2.rows}">
<body onload="saonloadp('<%=search %>',<%=nowPage %>,<c:out value='${row2.Pages}'/>)">
</c:forEach>
<div class="main">
    <div class="header">
        ...
        <dl id="guide">
            ...
            <dd>
                ...
                <a title="2MM图片" href="2mm.jsp"><span>2MM图片</span></a>
                <a title="美图录" href="meitulu.jsp"><span>美图录</span></a>
                <a title="RousiMM" href="rousi.jsp"><span>尤果网</span></a>
                <a title="第四印象" href="disi.jsp"><span>秀人网</span></a>
            </dd>
            <dt>名站套图</dt>
				<dd>
			...
			<dt>标签</dt>
                ...
                <a href="index.jsp"><img src="mobile/images/logo.png" alt="" class="logo"></a>
...
<div class="scroll_title">
			...
			</span> <span class="active"> <a href=""><%=search %></a>
			</span>
		</div>

<sql:query dataSource="${snapshot}" var="result1">
SELECT * FROM db_taotu where tagName like '<%=searcha %>' or name LIKE '<%=searcha %>' ORDER BY id DESC LIMIT <%=startnum %>,15;
</sql:query>
<div class="container">		
    		<div id="images">
