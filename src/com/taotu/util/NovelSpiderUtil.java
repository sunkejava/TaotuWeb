...
public final class NovelSpiderUtil {
	public static final Map<TaotuSiteEnum,Map<String,String>> CONTEXT_MAP = new HashMap<>();
	static{
		init();
	}
	...
	private static void init(){
		...
		try {
			Document doc =reader.read(new File("C:/WebConfig/TaotuWeb/Spider-Rule.xml"));
			...
			for(Element site : sites){
				...
				CONTEXT_MAP.put(TaotuSiteEnum.getEnumByUrl(subMap.get("url")), subMap);
			}
	 ...
	 * @return
	 */
	public static Map<String,String> getContext(TaotuSiteEnum taotuSiteEnum){
	...
	public static int getNovelStatus(String status){
		if(status.contains("连载")){
			...
			throw new RuntimeException("没有的图书状态！【"+status+"】");
		}
	}
	/**
	 * 把一个日期字符串转换为日期对象
